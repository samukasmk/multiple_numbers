...
class TestDisplayDetections(unittest.TestCase):
    ...
    def setUp(self):
        self.multiplicables_by_three_and_five = {15, 30, 45, 60, 75, 90}
        self.multiplicables_just_by_three = {i * 3 for i in range(1, 34)} - self.multiplicables_by_three_and_five
        self.multiplicables_just_by_five = {i * 5 for i in range(1, 21)} - self.multiplicables_by_three_and_five

        self.all_from_one_to_a_hundred = {i for i in range(1, 101)}
        self.non_multiplicables_by_three = self.all_from_one_to_a_hundred - self.multiplicables_just_by_three
